...
describe('testing the products', () => {
  ...
  it('should allow seller to create product', (done) => {
    chai
      .request(app)
      .post('/products/')
      .set('Authorization', `Bearer ${sellerToken}`)
  ...
  it('should allow admin to update product', (done) => {
    chai
      .request(app)
  ...
  it('should allow admin to change product availability', (done) => {
    chai
      .request(app)
      ...
      .end(async (err, res) => {
        chai.expect(res).to.have.status(201);
        chai.expect(res.body).to.have.property('product');
        // use the product variable here instead of reassigning it
        chai.expect(product.available).to.equal(false);
        done();
      });
  });
